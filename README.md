...
As an example, *Figure 2* displays the performance of various machine learning methods, i.e. LDA, Random Forest (Ranger), and XGBoost, in predicting food categories from the [cFMD](https://github.com/SegataLab/cFMD) database. The left panel shows the balanced accuracy of each method using 10-fold stratified outer resampling. The right panel provides the performance of the base learners across several metrics and a detailed breakdown for XGBoost across different food categories.

![Benchmark of base learners trained on a random 80% split data](data/cFMD_base_learners_benchmark.png)

<p align="center"><i>Figure 2: Benchmark of base learners trained on a random 80% split data.</i></p>

*Figure 3* provides an overview of the predictions made by LDA, Random Forest (Ranger), and XGBoost models on the remaining 20% of the samples. In this case, the models were trained on data where the minority food categories were synthetically oversampled, leading to significant improvement in predictive performance across all categories.

![Predictions on the remaining 20% cFMD dataset](data/cFMD_predictions.png)

<p align="center"><i>Figure 3: Predictions based on the remaining 20% of the cFMD samples.</i></p>

*Figure 4* presents the feature importance of the taxa used for training the XGBoost model. The importance is based on the contribution of each taxon to the predictive performance, highlighting key taxa that played a critical role in the classification of food categories.

![Feature importance of taxa for XGBoost](data/cFMD_feature_importance.png)

<p align="center"><i>Figure 4: Feature importance of taxa used for training the XGBoost model.</i></p>
...
- Carlino, N., Blanco-Míguez, A., Punčochář, M., Mengoni, C., Pinto, F., Tatti, A., Manghi, P., Armanini, F., Avagliano, M., Barcenilla, C., Breselge, S., Cabrera-Rubio, R., Calvete-Torre, I., Coakley, M., Cobo-Díaz, J. F., Filippis, F. D., Dey, H., Leech, J., Klaassens, E. S., … Pasolli, E. (2024). Unexplored microbial diversity from 2,500 food metagenomes and links with the human microbiome. Cell, 0(0). https://doi.org/10.1016/j.cell.2024.07.039
