...
tab_smote <- cbind(as.data.frame(smote_list$x_new), target = smote_list$y_new)
```
...
tab_smote %<>% sample_frac(size = 1)

# Random sampling on defined split
tab_train <-
  tab_smote %>% mutate(id = rownames(tab_smote)) %>% sample_frac(0.80)
tab_test  <-
  tab_smote %>% mutate(id = rownames(tab_smote)) %>% anti_join(tab_train, tab_smote, by = 'id') %>% select(-id)
tab_train$id <- NULL
